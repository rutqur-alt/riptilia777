...
## Критический баг-фикс: Объявления трейдера (03.03.2026)

### Проблема:
Объявление трейдера было видно в публичном стакане, но отсутствовало в разделе "Мои объявления". 
Это создавало риск финансовых потерь - трейдер не мог управлять своим объявлением.

### Причина:
1. **Frontend:** Фильтр `offers.filter(o => o.is_active || o.paused_by_admin)` скрывал неактивные объявления
2. **Backend:** Pydantic схема `OfferResponse` требовала обязательные поля без дефолтов
3. **Database:** Некоторые документы имели несогласованные поля (`status: "active"` + `is_active: false`)

### Исправления:
- ✅ **Frontend** (`TraderOffers.jsx`): Убран фильтр, теперь показываются ВСЕ объявления трейдера
- ✅ **Frontend**: Добавлены визуальные статусы "Активно"/"Закрыто" с затемнением неактивных
- ✅ **Backend** (`schemas.py`): Сделаны все поля `OfferResponse` опциональными с дефолтами
- ✅ **Backend** (`offers.py`): Улучшена функция `_normalize_offer()` для обработки всех полей
- ✅ **Migration**: Создан скрипт `/backend/migrations/normalize_offers.py` для нормализации данных

### Тестирование:
- Backend: 12/12 pytest тестов ✅
- Frontend: 8/8 playwright тестов ✅
- Тест-файлы: 
...
## Тестовый магазин с боевым подключением (03.03.2026)

### Реализовано:
- ✅ **Frontend** (`/app/frontend/src/pages/BuyerShop.jsx`):
  - Подключение через Invoice API v1 с HMAC-SHA256 подписью
  - Использует реальные API ключи мерчанта
  - callback_url направлен на тестовый приёмник вебхуков
  - Обработка ошибки "нет операторов" с понятным сообщением

- ✅ **WebhookPanel** (`/app/frontend/src/pages/shop/WebhookPanel.jsx`):
  - Панель для отображения входящих вебхуков в реальном времени
  - Показывает статус, сумму, время, ID платежа
  - Раскрываемый JSON payload при клике
  - Автообновление каждые 3 секунды
  - Кнопки обновления и очистки

- ✅ **Backend** (`/app/backend/routes/invoice_api.py`):
  - `POST /api/v1/invoice/test-webhook-receiver` — приёмник тестовых вебхуков
  - `GET /api/v1/invoice/test-webhooks` — получение списка вебхуков
  - `DELETE /api/v1/invoice/test-webhooks` — очистка вебхуков
  - Автоматическое сохранение вебхуков в `test_webhooks` коллекцию

### Что можно протестировать:
1. Создание платежа через Invoice API с подписью
2. Выбор оператора и способа оплаты
3. Создание сделки и просмотр реквизитов
4. Нажатие "Я оплатил" → вебхук `paid`
5. Завершение сделки → вебхук `completed`
6. Отмена → вебхук `cancelled`

## White-label интеграция (03.03.2026)

### Проблема:
Документация указывала, что клиент мерчанта должен переходить на наш домен (`payment_url`). Но требование — клиент НЕ должен видеть наш домен, всё на сайте мерчанта.

### Исправлено:

**Backend — новые endpoints для white-label:**
- `GET /api/v1/invoice/{id}/operators` — получить список операторов
- `POST /api/v1/invoice/{id}/select-operator` — выбрать оператора, получить реквизиты
- `POST /api/v1/invoice/{id}/mark-paid` — отметить как оплачено

**Frontend — обновлена документация:**
- Убраны упоминания `payment_url` и redirect'ов
- Добавлены примеры white-label интеграции (Python + JavaScript)
- Описан новый flow: create → operators → select-operator → mark-paid → webhooks

**Тестовый магазин:**
- Переключен на использование white-label API
- Все 3 объявления теперь отображаются
- Реквизиты (номер карты/телефона) отображаются корректно

**Исправлен баг:** У объявления 111111111 был `type: None` вместо `type: sell`, из-за чего оно не показывалось.
