...
@router.get("/stats")
async def get_trader_stats_endpoint(current_user: dict = Depends(get_current_user)):
    if current_user["role"] != "trader":
        raise HTTPException(status_code=403, detail="Only traders")
    stats = await get_trader_stats(current_user["id"])
    trader = await db.traders.find_one({"id": current_user["id"]}, {"_id": 0, "balance_usdt": 1, "frozen_usdt": 1})
    if trader:
        stats["balance_usdt"] = trader.get("balance_usdt", 0)
        stats["frozen_usdt"] = trader.get("frozen_usdt", 0)
    return stats
