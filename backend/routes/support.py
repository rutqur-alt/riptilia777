"""
Support routes - Support tickets and admin chat management
"""
...
async def send_ticket_message(ticket_id: str, data: MessageCreate, user: dict = Depends(get_current_user)):
...
async def admin_send_ticket_message(ticket_id: str, data: MessageCreate, user: dict = Depends(require_admin_level(30))):
...
async def admin_update_ticket_status(ticket_id: str, status: str, user: dict = Depends(require_admin_level(30))):
...
async def admin_approve_shop_application(ticket_id: str, shop_name: str = None, user: dict = Depends(require_admin_level(50))):
...
async def admin_reject_shop_application(ticket_id: str, reason: str = None, user: dict = Depends(require_admin_level(50))):
...
{f'Причина: {reason}' if reason else 'Если у вас есть вопросы, вы можете написать в этот чат.'}

Вы можете подать повторную заявку позже."""
