...
import { Store, Package, Plus, Trash2, Edit, Eye, EyeOff, Upload, Clock, CheckCircle, XCircle, AlertCircle, Download, Send, MessageCircle, Infinity, Image, FileText, Camera, Shield } from "lucide-react";
      ...
      <div className="max-w-2xl mx-auto">
        <div className="bg-[#121212] border border-white/5 rounded-2xl p-6">
          ...
          <div className="flex gap-3 mt-6">
            ...
            <Button onClick={createApplication} className="flex-1 bg-[#10B981] hover:bg-[#059669] text-white" title="Отправить сообщение">
              Отправить заявку
            </Button>
          </div>
        </div>
        ...
        <div className="border-t border-white/5 p-4">
          <div className="flex gap-2">
            <Input
              ...
              disabled={sending}
            />
            <Button onClick={handleSend} disabled={sending || !newMessage.trim()} className="bg-[#10B981] hover:bg-[#059669] text-white" title="Отправить сообщение">
        ...
        <div className="space-y-3">
          {filteredOrders.map(order => (
            <div key={order.id} className="bg-[#121212] border border-white/5 rounded-xl p-4">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-3">
                  ...
                  <span className={`px-2 py-0.5 text-xs rounded-full ${
                    ...
                    {order.status === "completed" ? "Выдан" : order.status === "pending" ? "Ожидает" : order.status === "pending_confirmation" ? "Ожидает подтверждения" : order.status === "cancelled" ? "Отменён" : order.status === "disputed" ? "Спор" : order.status}
                  </span>
                </div>
              ...
              {order.purchase_type === "guarantor" && (
                <div className="mt-3 pt-3 border-t border-white/5">
                  <button
                    onClick={() => navigate(`/trader/guarantor-chat/${order.id}`)}
                    className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-[#7C3AED]/10 text-[#A78BFA] hover:bg-[#7C3AED]/20 transition-colors text-xs font-medium"
                  >
...
function ShopMessagesTab({ token, shopName }) {
  ...
  const refreshAll = async () => {
    ...
    if (selectedConversation) {
      try {
        const resp = await axios.get(`${API}/shop/messages/${selectedConversation.id}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
...
// Withdraw Modal - transfers directly to account balance
function WithdrawModal({ token, balance, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(
        `${API}/shop/withdraw?amount=${amount}&method=to_balance&details=${encodeURIComponent("На баланс аккаунта")}`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
      <div className="bg-[#121212] border border-white/10 rounded-2xl p-6 w-full max-w-md">
        <h3 className="text-lg font-semibold text-white mb-4">Вывод на баланс аккаунта</h3>
        ...
        <p className="text-[#71717A] text-xs mb-4">
          Средства будут переведены на ваш основной баланс аккаунта
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3">
            ...
            >
              {loading ? "Перевод..." : "Вывести на баланс"}
            </Button>
          </div>
        </form>
      </div>
