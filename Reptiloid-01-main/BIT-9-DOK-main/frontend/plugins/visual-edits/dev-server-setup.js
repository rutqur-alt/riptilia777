...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*bitarbitr\.org$/)) {
        return true;
      }

      // Allow preview domains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*preview\.bitarbitr\.org$/)) {
        return true;
      }
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="system" -c user.email="system@bitarbitr.org" add "${targetFile}"`);
            execSync(`git -c user.name="system" -c user.email="system@bitarbitr.org" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
