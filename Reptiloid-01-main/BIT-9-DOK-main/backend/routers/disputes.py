...
async def confirm_payment_by_trader(
    ...
    # Финансовая логика - вызываем через API трейдера
    ...
    try:
        ...
        distribution = calculate_completion_distribution(
            original_amount_rub=original_amount_rub,
            fee_model=fee_model,
            trader_base_percent=5.0,
            merchant_fee_percent=merchant_fee_percent,
            marker_rub=order.get("marker", 0)
        )
        
        # Обновляем баланс трейдера
        trader_wallet = await _db.wallets.find_one({"user_id": trader["user_id"]})
        if trader_wallet:
            locked_usdt = order.get("amount_usdt", distribution.original_usdt)
            await _db.wallets.update_one(
                {"user_id": trader["user_id"]},
                {
                    "$inc": {
                        "locked_balance_usdt": -locked_usdt,
                        "available_balance_usdt": distribution.trader_earns_usdt
                    }
                }
            )
        ...
        logger.info(f"Dispute {dispute_id} resolved: trader earns {distribution.trader_earns_usdt} USDT")
    except Exception as e:
        logger.error(f"Financial calculation error: {e}")
        # Продолжаем без финансовой логики если ошибка
    ...
    return {"success": True, "message": "Платёж подтверждён, спор закрыт"}
