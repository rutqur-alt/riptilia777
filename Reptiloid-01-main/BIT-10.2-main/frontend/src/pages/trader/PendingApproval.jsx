...
import { DollarSign, Clock, MessageCircle, LogOut, RefreshCw } from 'lucide-react';
  ...
  const checkStatus = async () => {
    ...
    try {
      ...
      if (res.data.approval_status === 'approved' && res.data.is_active) {
        ...
        // Update local storage and reload to update context
        localStorage.setItem('user', JSON.stringify(res.data));
        window.location.href = '/trader';  // Full reload to update auth context
        ...
        <Card className="bg-zinc-900 border-zinc-800 max-w-lg w-full">
          <CardContent className="p-8 text-center">
            ...
            <p className="text-zinc-400 mb-6">
              Ваша заявка на регистрацию трейдера находится на рассмотрении администрации. 
            ...
            <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-4 mb-6 text-left">
              ...
              <ul className="text-sm text-zinc-300 space-y-2">
                ...
                <li className="flex items-start gap-2">
                  ...
                  После одобрения вы получите доступ к рабочему кабинету
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-emerald-400">3.</span>
                  Пополните депозит и начните принимать заявки
                </li>
              </ul>
            </div>
