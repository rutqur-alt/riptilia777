...
import { Copy, RefreshCw, Code, Book, Key, Shield, ExternalLink, Eye, EyeOff } from 'lucide-react';
...
const MerchantAPI = () => {
  ...
  const fetchApiDocs = async () => {
    try {
      const res = await api.get('/v1/invoice/docs');
      setApiDocs(res.data);
    } catch (error) {
  ...
  const regenerateApiKey = async () => {
    ...
    try {
      const res = await api.post('/merchant/regenerate-api-key');
  ...
  const regenerateSecretKey = async () => {
    ...
    try {
      const res = await api.post('/merchant/regenerate-secret-key');
      setMerchant(prev => ({ ...prev, secret_key: res.data.secret_key }));
  ...
  const BASE_URL = 'https://bitarbitr.org';
...
SECRET_KEY = "${merchant?.secret_key || merchant?.api_secret || 'YOUR_SECRET_KEY'}"
...
const SECRET_KEY = '${merchant?.secret_key || merchant?.api_secret || 'YOUR_SECRET_KEY'}';
  ...
  return (
    <DashboardLayout>
      <div className="max-w-5xl mx-auto space-y-6">
        {/* Header */}
        <div>
          <h1 className="text-2xl font-bold font-['Chivo']">API Интеграция</h1>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <div className="flex-1 bg-zinc-950 rounded-lg px-4 py-3 font-['JetBrains_Mono'] text-sm border border-zinc-800 truncate">
                    {merchant?.api_key}
                  </div>
                  <Button 
                    variant="outline" 
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <div className="flex-1 bg-zinc-950 rounded-lg px-4 py-3 font-['JetBrains_Mono'] text-sm border border-zinc-800 truncate">
                    {showSecret 
                      ? (merchant?.secret_key || merchant?.api_secret || 'Не установлен')
                      : '••••••••••••••••••••••••••••••••'
                    }
                  </div>
                  <Button 
                  ...
                  <Button 
                    ...
                    size="icon"
                    className="border-zinc-700 shrink-0"
                    onClick={() => copyToClipboard(merchant?.secret_key || merchant?.api_secret, 'Secret Key')}
          ...
          <CardContent className="py-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <span className="ml-2 font-['JetBrains_Mono'] text-emerald-400">{merchant?.id}</span>
              </div>
              <Button 
                variant="ghost" 
                size="sm"
        ...
        <Card className="bg-zinc-900 border-zinc-800">
          <CardHeader>
            <CardTitle className="text-lg font-['Chivo']">Invoice API v1 - Endpoints</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {[
                ...
                <div key={idx} className="flex items-center gap-4 p-3 bg-zinc-800/50 rounded-lg">
                  <span className={`px-2 py-1 rounded text-xs font-bold font-['JetBrains_Mono'] ${
                    endpoint.method === 'POST' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-blue-500/20 text-blue-400'
                  }`}>
                    {endpoint.method}
                  </span>
                  <code className="font-['JetBrains_Mono'] text-sm text-zinc-300 flex-1">
                    {endpoint.path}
                  </code>
        ...
        <Card className="bg-zinc-900 border-zinc-800">
          ...
          <CardContent>
            <div className="relative">
              <pre className="bg-zinc-950 rounded-lg p-4 overflow-x-auto text-sm font-['JetBrains_Mono'] text-zinc-300 border border-zinc-800">
                {signatureExample}
              </pre>
              <Button
                variant="ghost"
        ...
        <Card className="bg-zinc-900 border-zinc-800">
          ...
          <CardContent>
            <div className="relative">
              <pre className="bg-zinc-950 rounded-lg p-4 overflow-x-auto text-sm font-['JetBrains_Mono'] text-zinc-300 border border-zinc-800 max-h-[500px]">
        ...
        <Card className="bg-zinc-900 border-zinc-800">
          <CardHeader>
            <CardTitle className="text-lg font-['Chivo']">Webhook уведомления</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <pre className="bg-zinc-950 rounded-lg p-4 text-sm font-['JetBrains_Mono'] text-zinc-300 border border-zinc-800">
{`{
  "order_id": "ORDER_12345",
  "payment_id": "inv_20250124_ABC123",
  "status": "paid",
  "amount": 1500.00,
  "amount_usdt": 15.50,
  ...
  "sign": "f6g7h8i9j0..."
        ...
        <Card className="bg-zinc-900 border-zinc-800">
          <CardHeader>
            <CardTitle className="text-lg font-['Chivo']">Система споров</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <p className="text-sm text-zinc-400">
...
const sdk = new BitarbitrSDK({
  ...
  secretKey: '${merchant?.secret_key || merchant?.api_secret || 'YOUR_SECRET_KEY'}',
