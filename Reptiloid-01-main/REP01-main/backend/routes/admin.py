...
async def send_admin_dispute_message(trade_id: str, data: MessageCreate, user: dict = Depends(require_role(["admin"]))):
    """Admin sends message to dispute chat"""
...
async def get_dispute_details(trade_id: str, user: dict = Depends(require_role(["admin"]))):
