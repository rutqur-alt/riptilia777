...
async def apply_for_shop(data: ShopApplicationCreate, user: dict = Depends(get_current_user)):
    ...
    if user.get("role") != "trader":
        raise HTTPException(status_code=403, detail="Только трейдеры могут подать заявку на магазин")
...
async def get_my_shop_application(user: dict = Depends(get_current_user)):
    ...
    trader = await db.traders.find_one({"id": user["id"]}, {"_id": 0})
    if trader and trader.get("has_shop"):
        return {"has_shop": True, "shop_settings": trader.get("shop_settings", {}), "application": None}
...
async def review_shop_application(
    ...
    if decision == "approve":
        trader = await db.traders.find_one({"id": application["user_id"]}, {"_id": 0, "pending_shop_commission": 1})
...
async def admin_transfer(
    ...
    target = await db.traders.find_one({"id": target_user_id}, {"_id": 0})
    if not target:
...
async def request_shop_withdrawal(
    ...
    await db.traders.update_one(
        {"id": user["id"]},
        {"$inc": {"shop_balance": -amount, "shop_balance_reserved": amount}}
    )

    return {"status": "pending", "withdrawal_id": withdrawal_doc["id"]}
...
async def get_my_shop_conversations(user: dict = Depends(get_current_user)):
    """Get all shop conversations for current user"""
    conversations = await db.shop_conversations.find(
        {"customer_id": user["id"]},
        {"_id": 0}
    ).sort("updated_at", -1).to_list(100)
