...
async def get_user_detailed_stats(user_id: str, user: dict = Depends(require_admin_level(30))):
    ...
    # Find user
    trader = await db.traders.find_one({"id": user_id}, {"_id": 0, "password_hash": 0})
    merchant = await db.merchants.find_one({"id": user_id}, {"_id": 0, "password_hash": 0}) if not trader else None
...
async def get_all_shops(user: dict = Depends(require_admin_level(30))):
    ...
    traders = await db.traders.find({"has_shop": True}, {"_id": 0, "password_hash": 0}).to_list(500)
    shops = []
    for t in traders:
