...
async def get_super_admin_overview(user: dict = Depends(require_admin_level(80))):
    ...
    staff = await db.admins.find({}, {"_id": 0}).to_list(50)
...
async def toggle_maintenance_mode(enabled: bool, message: str = "", user: dict = Depends(require_admin_level(100))):
    ...
    await log_admin_action(user["id"], "toggle_maintenance", "system", "maintenance_mode", {"enabled": enabled})
...
async def get_all_users_detailed(
    ...
    if user_type in ["all", "staff"]:
        staff = await db.admins.find({}, {"_id": 0, "password_hash": 0}).to_list(50)
        for s in staff:
            s["user_type"] = "staff"
            s["status"] = "active"
            results.append(s)
...
async def ban_user(user_id: str, banned: bool, reason: str = "", duration_hours: int = None, user: dict = Depends(require_admin_level(50))):
    """Ban/unban user"""
...
async def adjust_user_balance(user_id: str, amount: float, reason: str = "", user: dict = Depends(require_admin_level(80))):
    """Adjust user balance"""
    trader = await db.traders.find_one({"id": user_id})
    if trader:
