...
async def get_marketplace_shops(category: Optional[str] = None, search: Optional[str] = None):
    """Get all active trader shops for marketplace"""
    result = []
...
async def get_my_marketplace_purchases(user: dict = Depends(get_current_user)):
    ...
    for purchase in purchases:
        seller = await db.traders.find_one({"id": purchase.get("seller_id")}, {"_id": 0, "nickname": 1})
        if seller:
            purchase["seller_nickname"] = seller.get("nickname", "Unknown")
