...
async def search_users(query: str, user: dict = Depends(get_current_user)):
    """Search users by nickname for messaging"""
    if len(query) < 2:
        return []
    ...
    traders = await db.traders.find({
        ...
        "id": {"$ne": user["id"]}
    }, {"_id": 0, "id": 1, "nickname": 1, "login": 1}).to_list(10)
    
    merchants = await db.merchants.find({
        "nickname": {"$regex": query, "$options": "i"},
        "id": {"$ne": user["id"]},
        "status": "active"
    }, {"_id": 0, "id": 1, "nickname": 1, "merchant_name": 1}).to_list(10)
    
    results = []
    for t in traders:
        results.append({"id": t["id"], "nickname": t["nickname"], "type": "user"})
    for m in merchants:
        results.append({"id": m["id"], "nickname": m["nickname"], "type": "merchant", "name": m.get("merchant_name")})
