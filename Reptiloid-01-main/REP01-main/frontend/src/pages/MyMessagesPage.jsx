/**
 * My Messages Page - Unified messaging for users
 * Based on СООБЩЕНИЯ.txt specification
 * 
 * Categories:
 * 1. P2P Сделки (p2p_trade, p2p_merchant)
 * 2. Маркетплейс Заказы (marketplace)
 * 3. Поддержка (support_ticket)
 * 4. Форум (forum_topic)
 * 
...
// Category configuration per spec
const CATEGORIES = {
  all: { label: 'Все', icon: MessageCircle, color: 'text-[#A1A1AA]' },
  p2p: { label: 'P2P Сделки', icon: ArrowRightLeft, color: 'text-[#10B981]', types: ['p2p_trade', 'p2p_merchant', 'p2p_dispute'] },
  marketplace: { label: 'Маркетплейс', icon: ShoppingBag, color: 'text-[#8B5CF6]', types: ['marketplace', 'marketplace_guarantor', 'shop_application'] },
  support: { label: 'Поддержка', icon: HelpCircle, color: 'text-[#3B82F6]', types: ['support_ticket'] },
  forum: { label: 'Форум', icon: Users, color: 'text-[#F59E0B]', types: ['forum_topic'] }
};

const TYPE_CONFIG = {
  p2p_trade: { icon: ArrowRightLeft, label: 'P2P Сделка', color: 'text-[#10B981]', category: 'p2p' },
  p2p_merchant: { icon: ArrowRightLeft, label: 'Сделка мерчанта', color: 'text-[#F97316]', category: 'p2p' },
  p2p_dispute: { icon: AlertTriangle, label: 'Спор P2P', color: 'text-[#EF4444]', category: 'p2p', isDispute: true },
  marketplace: { icon: ShoppingBag, label: 'Заказ маркетплейса', color: 'text-[#8B5CF6]', category: 'marketplace' },
  marketplace_guarantor: { icon: Shield, label: 'Гарант-сделка', color: 'text-[#A855F7]', category: 'marketplace' },
  shop_application: { icon: ShoppingBag, label: 'Заявка на магазин', color: 'text-[#F97316]', category: 'marketplace' },
  support_ticket: { icon: HelpCircle, label: 'Обращение в поддержку', color: 'text-[#3B82F6]', category: 'support' },
  forum_topic: { icon: Users, label: 'Тема форума', color: 'text-[#F59E0B]', category: 'forum' }
};
...
export default function MyMessagesPage() {
  ...
  const filteredConversations = conversations.filter(conv => {
    ...
    return categoryConfig?.types?.includes(conv.type);
  });

  // Count conversations per category
  const categoryCounts = {
    all: conversations.length,
    p2p: conversations.filter(c => ['p2p_trade', 'p2p_merchant'].includes(c.type)).length,
    marketplace: conversations.filter(c => c.type === 'marketplace').length,
    support: conversations.filter(c => c.type === 'support_ticket').length,
    forum: conversations.filter(c => c.type === 'forum_topic').length
  };
  ...
  return (
    <div className="space-y-4" data-testid="my-messages-page">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-white">Мои сообщения</h1>
          <p className="text-[#71717A] text-sm">Сделки, заказы и обращения в поддержку</p>
        </div>
        <Button
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              ...
              <div className="p-6 text-center">
                ...
                <p className="text-[#71717A] text-sm">Нет чатов в этой категории</p>
