...
import { Wallet, ArrowLeft, Gamepad2, ShoppingBag, Video, HelpCircle, MessageCircle, CheckCircle, Clock } from "lucide-react";

const merchantTypes = [
  { id: "casino", name: "Казино", icon: Gamepad2, description: "Онлайн-казино и беттинг", commission: "0.5%" },
  { id: "shop", name: "Магазин", icon: ShoppingBag, description: "Интернет-магазины", commission: "0.3%" },
  { id: "stream", name: "Стрим", icon: Video, description: "Стриминговые платформы", commission: "0.4%" },
  { id: "other", name: "Другое", icon: HelpCircle, description: "Другие типы бизнеса", commission: "0.6%" }
];
    ...
    <div className="min-h-screen bg-[#0A0A0A]">
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        ...
        <div className="flex items-center justify-center gap-4 mb-12">
          <div className={`flex items-center gap-2 ${step >= 1 ? "text-[#7C3AED]" : "text-[#52525B]"}`}>
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold ${step >= 1 ? "bg-[#7C3AED] text-white" : "bg-[#27272A] text-[#71717A]"}`}>
              1
            </div>
            <span className="hidden sm:inline">Тип бизнеса</span>
          </div>
          <div className={`w-12 h-px ${step >= 2 ? "bg-[#7C3AED]" : "bg-[#27272A]"}`} />
          <div className={`flex items-center gap-2 ${step >= 2 ? "text-[#7C3AED]" : "text-[#52525B]"}`}>
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold ${step >= 2 ? "bg-[#7C3AED] text-white" : "bg-[#27272A] text-[#71717A]"}`}>
              2
            </div>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                {merchantTypes.map((type) => (
                  <button
                    key={type.id}
                    type="button"
                    onClick={() => setFormData({ ...formData, merchant_type: type.id })}
                    className={`p-6 rounded-2xl border-2 text-left transition-all ${
                      formData.merchant_type === type.id
                        ? "border-[#7C3AED] bg-[#7C3AED]/10"
                        : "border-white/10 bg-[#121212] hover:border-white/20"
                    }`}
                    data-testid={`merchant-type-${type.id}`}
                  >
                    <type.icon className={`w-8 h-8 mb-4 ${formData.merchant_type === type.id ? "text-[#7C3AED]" : "text-[#71717A]"}`} />
                    <div className="text-lg font-semibold text-white mb-1">{type.name}</div>
                    <div className="text-sm text-[#71717A] mb-3">{type.description}</div>
                    <div className="inline-flex items-center gap-1 px-2 py-1 bg-[#10B981]/10 rounded-md">
                      <span className="text-xs text-[#10B981]">Комиссия: {type.commission}</span>
                    </div>
                  </button>
                ))}
              </div>
              ...
              <Button
                ...
                onClick={() => {
                  if (!formData.merchant_type) {
                    toast.error("Выберите тип площадки");
                    return;
                  }
                  setStep(2);
                }}
            ...
            <div className="space-y-6 animate-fade-in-up">
                ...
                <div className="grid sm:grid-cols-2 gap-5">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      data-testid="merchant-login-input"
                      required
                    />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      id="merchant_name"
                      type="text"
                      placeholder="Например: Stake Casino"
                ...
                <div className="grid sm:grid-cols-2 gap-5">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      className="bg-[#1A1A1A] border-white/10 text-white placeholder:text-[#52525B] h-12 rounded-xl focus:border-[#7C3AED]"
                      data-testid="merchant-password-input"
                      required
                    />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      className="bg-[#1A1A1A] border-white/10 text-white placeholder:text-[#52525B] h-12 rounded-xl focus:border-[#7C3AED]"
                      data-testid="merchant-confirm-password-input"
                      required
                    />
                  </div>
                </div>
              ...
              <div className="flex gap-4">
                <Button
                  ...
                  onClick={() => setStep(1)}
                  variant="outline"
                  className="flex-1 border-white/10 text-white hover:bg-white/5 h-12 rounded-xl"
                >
                  Назад
                </Button>
                <Button
                  ...
                  className="flex-1 bg-[#7C3AED] hover:bg-[#6D28D9] text-white h-12 rounded-xl font-semibold disabled:opacity-50"
                  data-testid="submit-merchant-btn"
                >
