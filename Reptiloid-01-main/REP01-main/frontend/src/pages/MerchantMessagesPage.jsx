/**
 * Merchant Messages Page - Unified messaging for merchants
 * Exact copy of MyMessagesPage but with merchant-specific API endpoints
 * 
 * Categories:
 * 1. P2P Сделки (p2p_trade, p2p_merchant, crypto_order)
 * 2. Маркетплейс (marketplace, marketplace_guarantor)
...
const CATEGORIES = {
  ...
  p2p: { label: 'P2P Сделки', icon: ArrowRightLeft, color: 'text-[#10B981]', types: ['p2p_trade', 'p2p_merchant', 'crypto_order', 'p2p_dispute'] },
  marketplace: { label: 'Маркетплейс', icon: ShoppingBag, color: 'text-[#8B5CF6]', types: ['marketplace', 'marketplace_guarantor'] },
  support: { label: 'Поддержка', icon: HelpCircle, color: 'text-[#3B82F6]', types: ['support_ticket', 'merchant_application'] },
  forum: { label: 'Форум', icon: Users, color: 'text-[#F59E0B]', types: ['forum_topic'] }
};

const TYPE_CONFIG = {
  p2p_trade: { icon: ArrowRightLeft, label: 'P2P Сделка', color: 'text-[#10B981]' },
  p2p_merchant: { icon: ArrowRightLeft, label: 'Сделка мерчанта', color: 'text-[#F97316]' },
  p2p_dispute: { icon: AlertTriangle, label: 'Спор P2P', color: 'text-[#EF4444]', isDispute: true },
  crypto_order: { icon: ArrowRightLeft, label: 'Крипто заказ', color: 'text-[#F59E0B]' },
  marketplace: { icon: ShoppingBag, label: 'Заказ маркетплейса', color: 'text-[#8B5CF6]' },
  marketplace_guarantor: { icon: ShoppingBag, label: 'Гарант сделка', color: 'text-[#8B5CF6]' },
  support_ticket: { icon: HelpCircle, label: 'Обращение в поддержку', color: 'text-[#3B82F6]' },
  forum_topic: { icon: Users, label: 'Тема форума', color: 'text-[#F59E0B]' },
...
export default function MerchantMessagesPage() {
  ...
  const categoryCounts = {
    ...
    p2p: conversations.filter(c => ['p2p_trade', 'p2p_merchant', 'crypto_order'].includes(c.type)).length,
    marketplace: conversations.filter(c => ['marketplace', 'marketplace_guarantor'].includes(c.type)).length,
