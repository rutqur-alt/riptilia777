...
export default function BuyerTradePage() {
  ...
  const initConversation = async () => {
    try {
      const response = await axios.post(`${API}/msg/trade/${tradeId}/init`, {}, {
      ...
      // Fallback
      try {
        const convResponse = await axios.get(`${API}/msg/conversations`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        const tradeConv = convResponse.data?.find(c => c.related_id === tradeId);
        if (tradeConv) {
          setConversationId(tradeConv.id);
        }
      } catch (e) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      await axios.post(`${API}/msg/conversations/${conversationId}/send`, {
        content: newMessage
      }, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleDispute = async () => {
    ...
    try {
      ...
      await axios.post(`${API}/msg/trade/${tradeId}/dispute`, { reason }, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleLeaveChat = async () => {
    if (!conversationId) return;
    try {
      await axios.post(`${API}/msg/user/conversation/${conversationId}/leave`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      <header className="bg-[#121212] border-b border-white/5 px-4 py-4">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-lg font-bold text-white">Покупка #{trade.id.slice(0, 12)}</h1>
                ...
                <>
                  ...
                  {canDispute ? (
                    <Button 
                      ...
                      variant="outline"
                      className="w-full border-[#F59E0B]/50 text-[#F59E0B] hover:bg-[#F59E0B]/10 h-10 rounded-xl"
              ...
              {trade.status === "disputed" && (
                <div className="bg-[#EF4444]/10 rounded-xl p-3">
                  <div className="flex items-center gap-2 text-[#EF4444] text-sm">
                    ...
                    <span>Спор открыт. Модератор рассмотрит сделку.</span>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="p-4 border-t border-white/5">
              <div className="flex gap-2">
                ...
                <Button 
                  ...
                  disabled={!newMessage.trim() || sendingMessage || !conversationId}
