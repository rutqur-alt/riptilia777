      ...
      if (purchaseType === "instant" && response.data.delivered_content) {
        ...
        toast.success(
          <div>
            ...
            {contentItems.some(item => item?.file_url || item?.photo_url) && (
              <div className="text-xs mt-2 text-blue-300">
                📎 Файлы доступны в разделе "Мои покупки"
              </div>
            )}
          </div>,
          { duration: 30000 }
        ...
        navigate("/trader?section=market&subsection=purchases");
      } else {
    ...
    return (
      <div className="min-h-screen bg-[#0A0A0A] flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/marketplace">
            <Button className="mt-4 bg-[#10B981] hover:bg-[#059669]">Назад</Button>
          </Link>
        </div>
      </div>
    );
