...
export function DisputeChat({ 
  ...
  const initConversation = async () => {
    try {
      ...
      const initRes = await axios.post(`${API}/msg/trade/${tradeId}/init`, {}, {
      ...
      const convRes = await axios.get(`${API}/msg/conversations/${convId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const onWsTradeMessage = useCallback((data) => {
    if (data.type === "message") {
      ...
      setConversation(prev => prev ? { ...prev, status: data.status || prev.status } : prev);
    }
  }, []);

  useWebSocket(
    conversation ? `/ws/conversation/${conversation.id}` : null,
    onWsTradeMessage,
    { enabled: !!conversation && conversation.status !== 'completed' }
  );

  useEffect(() => {
    if (tradeId && token) {
      initConversation();
      const interval = setInterval(initConversation, 30000);
  ...
  const sendMessage = async () => {
    ...
    try {
      await axios.post(
        `${API}/msg/conversations/${conversation.id}/send`,
        { content: newMessage },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setNewMessage('');
...
export function SupportTicketChat({ 
  ...
  const onWsConvMessage = useCallback((data) => {
    if (data.type === "message") {
      setMessages(prev => {
        const exists = prev.some(m => m.id === data.id);
        if (exists) return prev;
...
export function StaffChatComponent({ token, currentUserId, currentUserRole }) {
  ...
  const fetchStaffChats = async () => {
    try {
      const response = await axios.get(`${API}/msg/staff/chats`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const fetchGeneralChat = async () => {
    try {
      const response = await axios.get(`${API}/msg/staff/general`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const onWsStaffMessage = useCallback((data) => {
    if (data.type === "message") {
      setMessages(prev => {
        const exists = prev.some(m => m.id === data.id || (m.content === data.content && m.sender_id === data.sender_id));
        if (exists) return prev;
        return [...prev, data];
      });
    }
  ...
  useWebSocket(
    selectedConv && selectedConv.id !== 'general' ? `/ws/conversation/${selectedConv.id}` : null,
    onWsStaffMessage,
    { enabled: !!selectedConv && selectedConv.id !== 'general' }
  );
...
export function ApplicationChat({
  ...
  const fetchChat = async () => {
    try {
      const response = await axios.get(
        `${API}/msg/application/${applicationType}/${userId}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setConversation(response.data.conversation);
      setMessages(response.data.messages || []);
  ...
  const onWsAppMessage = useCallback((data) => {
    if (data.type === "message") {
      setMessages(prev => {
        const exists = prev.some(m => m.id === data.id);
        if (exists) return prev;
