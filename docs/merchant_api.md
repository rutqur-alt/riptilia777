...
Merchant API позволяет интегрировать приём платежей через P2P-трейдеров на вашем сайте. Клиент **никогда не покидает ваш сайт** — весь процесс оплаты происходит через API.

### Схема работы

```
1. Клиент выбирает товар на вашем сайте
2. Ваш сервер создаёт счёт через API → получает invoice_id
3. Ваш сервер запрашивает список операторов → показывает клиенту
4. Клиент выбирает оператора → создаётся trade
5. Ваш сервер запрашивает реквизиты → показывает клиенту
6. Клиент переводит деньги по реквизитам
7. Клиент нажимает "Я оплатил" → ваш сервер вызывает mark-paid
8. Оператор подтверждает → вы получаете webhook "completed"
9. Начисляете товар/услугу клиенту
```
...
```json
{
    ...
    "payment_url": "/shop/pay/INV_20241215123456_A1B2C3D4"
}
```
    ...
    "requisites": [
        {
            ...
            "bank_name": "Сбербанк",
            "card_holder": "Иван Петров"
        }
    ],
    "trader_login": "CryptoTrader"
}
```
