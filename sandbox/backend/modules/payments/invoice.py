...
def _validate_callback_url(url: str) -> bool:
    """Validate callback_url is not targeting internal network (SSRF protection)"""
    try:
        parsed = urlparse(url)
        ...
        try:
            ...
            # It's a domain name, try to resolve
            try:
                ...
                pass  # Can't resolve - allow (might be created later)
        return True
    except Exception:
        return False
...
# Поля для подписи вебхуков (отправка мерчанту) — должны совпадать с документацией в ЛК
WEBHOOK_SIGN_FIELDS = ['order_id', 'payment_id', 'status', 'amount', 'amount_usdt', 'timestamp']

def generate_signature(data: Dict[str, Any], secret_key: str, sign_fields: list = None) -> str:
    """Генерация HMAC-SHA256 подписи"""
    fields = sign_fields or SIGN_FIELDS
    sign_data = {}
    for k, v in data.items():
        # Используем только поля из указанных sign_fields
        if k not in fields or k == 'sign' or v is None:
            continue
...
async def send_webhook_notification(invoice_id: str, new_status: str, extra_data: dict = None):
    """Отправляет webhook-уведомление мерчанту (единый формат для P2P и QR)"""
    try:
        ...
        # Получаем данные связанной сделки для расширенного payload
        trade = await db.trades.find_one({"payment_link_id": invoice_id}, {"_id": 0})
        
        amount_rub = invoice.get("original_amount_rub") or invoice.get("amount_rub")
        amount_usdt = invoice.get("amount_usdt")
        
        # Получаем базовый курс платформы (Rapira) из payout_settings
        ...
        # Рассчитываем данные из сделки
        commission_percent = None
        commission_usdt = None
        commission_rub = None
        merchant_received_usdt = None
        merchant_received_rub = None
        original_amount_rub = None
        client_paid_rub = None
        trader_rate = None
        trade_id = None
        
        if trade:
            trade_id = trade.get("id")
            trader_rate = trade.get("price_rub")
            original_amount_rub = trade.get("original_amount_rub") or amount_rub
            client_paid_rub = trade.get("client_pays_rub") or trade.get("amount_rub")
            merchant_received_rub = trade.get("merchant_receives_rub")
            merchant_received_usdt_val = trade.get("merchant_receives_usdt")
            if merchant_received_usdt_val is not None:
                merchant_received_usdt = round(float(merchant_received_usdt_val), 6)
            commission_percent_val = trade.get("merchant_commission_percent")
            if commission_percent_val is not None:
                commission_percent = float(commission_percent_val)
            platform_fee = trade.get("platform_fee_rub")
            if platform_fee is not None:
                commission_rub = float(platform_fee)
            trade_amount_usdt = trade.get("amount_usdt")
            if merchant_received_usdt is not None and trade_amount_usdt:
                commission_usdt = round(float(trade_amount_usdt) - merchant_received_usdt, 6)
        ...
        # Сначала применяем extra_data (данные от вызывающего кода)
        if extra_data:
            callback_data.update(extra_data)
        
        # Расширенные данные для бухгалтерии (рассчитанные — имеют приоритет)
        callback_data["base_rate"] = base_rate
        if trade_id:
            callback_data["trade_id"] = trade_id
        if trader_rate is not None:
            callback_data["trader_rate"] = trader_rate
        if original_amount_rub is not None:
            callback_data["original_amount_rub"] = original_amount_rub
        if client_paid_rub is not None:
            callback_data["client_paid_rub"] = client_paid_rub
        if merchant_received_usdt is not None:
            callback_data["merchant_received_usdt"] = merchant_received_usdt
        if merchant_received_rub is not None:
            callback_data["merchant_received_rub"] = float(merchant_received_rub)
        if commission_percent is not None:
            callback_data["commission_percent"] = commission_percent
        if commission_usdt is not None:
            callback_data["commission_usdt"] = commission_usdt
        if commission_rub is not None:
            callback_data["commission_rub"] = commission_rub
        
        # Время завершения/оплаты в московском времени
        if extra_data and extra_data.get("completed_at"):
            try:
                completed_utc = datetime.fromisoformat(str(extra_data["completed_at"]).replace('Z', '+00:00'))
                callback_data["completed_at"] = completed_utc.astimezone(MSK).isoformat()
            except (ValueError, TypeError):
                pass
        if extra_data and extra_data.get("paid_at"):
            try:
                paid_utc = datetime.fromisoformat(str(extra_data["paid_at"]).replace('Z', '+00:00'))
                callback_data["paid_at"] = paid_utc.astimezone(MSK).isoformat()
            except (ValueError, TypeError):
                pass
        
        # Убираем внутренние поля, не предназначенные для мерчанта
        INTERNAL_FIELDS = [
            "qr_aggregator", "merchant_receives_usdt",
            "client_amount_rub", "rate",
            "provider_id", "trader_id", "trader_login",
            "offer_id", "payment_link_id", "buyer_id",
            "buyer_type", "qr_price", "total_freeze_usdt",
            "platform_commission_usdt", "provider_commission_usdt",
            "exchange_rate"
        ]
        for internal_field in INTERNAL_FIELDS:
            callback_data.pop(internal_field, None)
        
        # Подпись по полям из документации (WEBHOOK_SIGN_FIELDS)
        callback_data["sign"] = generate_signature(callback_data, secret_key, WEBHOOK_SIGN_FIELDS)
        ...
        # Send webhook
        result = await send_webhook(callback_url, callback_data, 0)
        
        # Детальный результат доставки
        update_data = {
            "status": "delivered" if result["success"] else "retry_scheduled",
        ...
        # Mark invoice as webhook_sent when successfully delivered
        if result["success"]:
            await db.merchant_invoices.update_one(
                {"id": invoice_id},
                {"$set": {"webhook_sent": True, "webhook_sent_at": datetime.now(timezone.utc).isoformat()}}
            )
...
async def send_webhook(callback_url: str, payload: Dict[str, Any], retry_count: int = 0) -> Dict[str, Any]:
    """Отправка webhook с retry логикой. Возвращает детальный результат."""
    result = {
        "success": False,
        "status_code": None,
...
async def create_invoice(
    ...
    # 1.1 Check merchant traffic status
    if not merchant.get("traffic_enabled", False):
        raise HTTPException(status_code=403, detail={
            "status": "error",
            "code": "MERCHANT_TRAFFIC_DISABLED",
            "message": "Трафик мерчанта отключён. Создание новых платежей запрещено."
        })
    ...
    # 4.5 Validate callback_url (SSRF protection)
    if not _validate_callback_url(request.callback_url):
        raise HTTPException(status_code=400, detail={
            "status": "error",
            "code": "INVALID_CALLBACK_URL",
            "message": "callback_url не может указывать на внутренний адрес"
        })
    ...
    # Используем PAYMENT_DOMAIN для формирования payment_url
    import os
    payment_domain = os.environ.get("PAYMENT_DOMAIN", "https://secure-payments.st")
    
    # URL на страницу выбора оператора
    payment_url = f"{payment_domain}/select-operator/{invoice_id}"
...
async def select_operator_for_invoice(
    ...
    from modules.p2p.trades import send_merchant_webhook_on_trade
...
async def get_merchant_transactions(
    ...
    def get_created_at_str(x):
        val = x.get("created_at", "")
        if hasattr(val, 'isoformat'):
            return val.isoformat()
        return str(val) if val else ""
    all_transactions.sort(key=get_created_at_str, reverse=True)
    all_transactions = all_transactions[:limit]
    
    # Filter sensitive/internal fields before returning to merchant
    ALLOWED_TRANSACTION_FIELDS = {
        "id", "status", "type", "amount_rub", "amount_usdt", "original_amount_rub",
        "currency", "created_at", "completed_at", "paid_at", "expires_at",
        "external_order_id", "description", "requested_payment_method",
        "fee_model", "merchant_fee_percent", "exchange_rate", "marker",
        "merchant_id", "invoice_id", "trade_number",
        "merchant_receives_usdt", "merchant_receives_rub",
        "merchant_commission_percent", "merchant_commission",
        "webhook_sent", "callback_url"
    }
    
    def _sanitize_transaction(tx):
        """Remove internal/sensitive fields from transaction data"""
        return {k: v for k, v in tx.items() if k in ALLOWED_TRANSACTION_FIELDS}
    
    sanitized = [_sanitize_transaction(tx) for tx in all_transactions]
...
async def test_webhook_receiver(request: Request):
    ...
    Only accepts requests from localhost (internal webhook delivery).
    """
    # Security: only allow internal webhook delivery
